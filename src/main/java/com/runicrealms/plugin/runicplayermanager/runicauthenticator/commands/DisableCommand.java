...
public final class DisableCommand extends AuthCommand {
    public DisableCommand(Main instance) {
        super(instance, "disable", "runicauth.disable", "Disables 2FA on an account");
    }
                ...
                public void run() {
                    ...
                    if (player.isOnline()) {
                        ...
                        getInstance().sync(new Runnable() {
                            @Override
                            public void run() {
                                getInstance().getC().send(onlinePlayer, getInstance().getC().message("otherDisable").replaceAll("%player%", commandSender.getName()));
                                getInstance().getC().sendDirect(commandSender, "&7You have disabled 2FA on " + player.getName() + "'s account.");
                                u.invalidateKey(onlinePlayer);
                                u.reverseInventory(onlinePlayer);
                                getInstance().save();
                            }
                        });
                    } else {
